...
volatile char ADC_In_Ready = 0;;
unsigned short ADCT0A_In(void){
	ADC_In_Ready = 0;
	while (!(ADC_In_Ready));
	return ADCvalue;
}

volatile unsigned char ADC_Collect_Complete = 0;
/*int ADC_Collect(unsigned char channelNum, unsigned int fs,
  unsigned long * buffer, unsigned int numberOfSamples){
		
		int count;
		int prescale = 249, period; // 50MHz=(prescale+1)/(period+1)
		                          // period[0..65535], prescale[0..255]
		                          // prescale fixed at 249+1
		                          // period varies from 20 to 2000
		                          // to get sampling rates from 10000 to 10Hz
		                          // period=(50MHz/250/fs)-1=(200000/fs)-1
		period=(200000/fs)-1;
		
		ADC_Collect_Complete = 0;
		
		//Initialize ADC0 with desired frequency
...
void ADC_Collect(unsigned int channelNum, unsigned int fs, void (*task)(unsigned long)){
	//int count;
		int prescale = 49, period; // 80MHz=(prescale+1)/(period+1)
		                          ...
		                          // period varies from 20 to 2000
		                          // to get sampling rates from 10000 to 10Hz
		                          // period=(80MHz/50/fs)-1=(1600000/fs)-1
		period=(1600000/fs)-1;
	  ADC_task = task;
	  global_channel = channelNum;
	  global_fs = fs;
	  ADC0_InitTimer0ATriggerSeq3(channelNum, prescale, period);
}
...
void ADC0Seq3_Handler(void){
  ...
  ADCvalue = ADC0_SSFIFO3_R&ADC_SSFIFO3_DATA_M;
	...
	ADC_task(ADCvalue);
	//UART_OutChar('2');
	//ADC_Collect(global_channel, global_fs, ADC_task);
}
