...
void Wait1ms(unsigned long delay){
	unsigned long volatile time;
  while(delay){
    time = 727240*2/91; // 1msec <- I got this number by calibrated with the scope
    while(time){
    time--;
    }
   delay--;
...
void ST7735_Message (int device, int line, char *string, unsigned long value){
  ...
  while (i<length && i<12){
		ST7735_DrawChar(begin_x+i*6*FONT_SIZE, begin_y, *(string + i), ST7735_Color565(255, 0, 0), 0, FONT_SIZE); 
		i++;
	}	 
	
	for (;i<12;i++)
		ST7735_DrawChar(begin_x+i*6*FONT_SIZE, begin_y, ' ', ST7735_Color565(255, 0, 0), 0, FONT_SIZE); 
	/*
	if (value<0) {
		ST7735_DrawChar(begin_x+i*6*FONT_SIZE, begin_y, '-', ST7735_Color565(255, 0, 0), 0, FONT_SIZE); 
		value*=-1;
	...
	*/
	...
	number=value;
	for (;digits>0;digits--){
		ST7735_DrawChar(begin_x+i*6*FONT_SIZE, begin_y, (number/power)+48, ST7735_Color565(255, 0, 0), 0, FONT_SIZE); 
		number%=power;
		power/=10;
		i++;
	}
